...
int inizializza(){
    ...
    if (dbClienti == NULL) return 1;
         ...
         while (fgets(infoClienti, 100, dbClienti) != NULL){
               ...
               while(datoCorrente <= 6) { //Esistono 7 campi, dallo 0 al 6.
					...
					switch(datoCorrente){
						...
						case 5: strcpy(nuovoCliente.cartaCredito, decripta(prendiChiave(), campoTemp, 16)); break;
    ...
    printf("\t3. Controllo l'algoritmo di crittografia...\n");
  	char crittografa[17] = "6100675866256849\0";
  	printf("\t\tStringa di partenza: %s\n", crittografa);
  	printf("\t\tStringa criptata: %s\n", cripta(prendiChiave(), crittografa, 16));
  	printf("\t\tStringa decriptata: %s\n", decripta(prendiChiave(), cripta(prendiChiave(), crittografa, 16), 16));
